...
/** Formats an ISO date string to "June 18, 2026" */
...
function buildCustomerEmail(ticket: TicketPayload): SendEmailCommandInput["Message"] {
  const lines = [
  ...
  if (ticket.purchaseDate) lines.push(`Purchase Date:     ${formatDate(ticket.purchaseDate)}`)
  if (ticket.amazonOrderId) lines.push(`Amazon Order ID:   ${ticket.amazonOrderId}`)
  ...
  return {
    Subject: {
      Data: `[mokhaLab] Support Request Received — ${ticket.ticketId}`,
      Charset: "UTF-8",
    },
...
function buildAdminEmail(ticket: TicketPayload): SendEmailCommandInput["Message"] {
  const lines = [
  ...
  if (ticket.photoCount) lines.push("", `Photos uploaded: ${ticket.photoCount}`)

  lines.push("", `Submitted: ${ticket.submissionTimestamp}`)

  return {
    Subject: {
      Data: `[mokhaLab Support] New Ticket — ${ticket.ticketId} (${ticket.caseType})`,
      Charset: "UTF-8",
    },
    Body: { Text: { Data: lines.join("\n"), Charset: "UTF-8" } },
  }
...
export const handler: APIGatewayProxyHandlerV2 = async (event) => {
  ...
  const full = ticket as TicketPayload
    ...
    await Promise.all([
      ses.send(
        new SendEmailCommand({
          Source: fromEmail,
          Destination: { ToAddresses: [full.email] },
          ReplyToAddresses: [adminEmail],
          Message: buildCustomerEmail(full),
        })
      ),
      ses.send(
        new SendEmailCommand({
          ...
          ReplyToAddresses: [full.email],
          Message: buildAdminEmail(full),
        })
      ),
    ])

    return ok({ ok: true, ticketId: full.ticketId })
  } catch (e) {
